...
Module lifted to `plugins.zetesis.research.search_backend`. This shim aliases the
old import path to the new module via `sys.modules` so every existing
`from ops.benchmarks.adr_010.harness.search_backend import X` and
`from ops.benchmarks.adr_010.harness import search_backend` call site continues to
...
from plugins.zetesis.research import search_backend as _mod
