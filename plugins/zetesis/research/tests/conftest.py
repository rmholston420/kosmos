"""Auto-installed test fixtures for the Zetesis research inner loop.

Formerly under ``ops/benchmarks/adr_010/tests/conftest.py`` — moved
with the ODR-side tests to ``plugins/zetesis/research/tests/`` per
ADR-056 §D1 (Stage 6.3 (proper) sub-slice 1).
...
otherwise touch the live network. This conftest stubs the fetch autouse
...
def _stub_enterprise_license_grounding(monkeypatch, request):
    ...
    if request.node.get_closest_marker("no_stub_enterprise_license"):
        return
    from plugins.zetesis.research import enterprise_license_grounding
